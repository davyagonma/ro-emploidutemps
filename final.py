...
def generate_pdf(emploi_temps):
    ...
    pdf.cell(200, 10, "MASTER 1 EN GENIE LOGICIEL", ln=True, align='C')
    ...
    jours = ['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi']
    horaires = ["08:00-10:00", "10:15-12:15", "12:15-14:00", "14:00-16:00", "16:15-18:15"]
    column_widths = [40] + [35] * len(jours)
    
    pdf.set_font("Arial", style='B', size=10)
    for col, width in zip(["Heure"] + jours, column_widths):
    ...
    for h in horaires:
        ...
        for j in jours:
            ...
            texte = "\n".join([f"{row['Cours']}\n{row['Professeur']} - {row['Salle']}" for _, row in cours_info.iterrows()])
            pdf.cell(35, 10, texte if not cours_info.empty else "", border=1, align='C')
        pdf.ln()
